...
#   /data01/drex_runs/datasets/a2261/Images/
# and catalogs from:
#   /data01/drex_runs/datasets/a2261/Catalogs/
# and create output here:
#   /data02/drex_runs/a2261/drex/v8/110612/
#
# These directories can be overridden with the options:
# -indir my_indir -- imports catalogs from my_indir
# -imdir my_imdir -- imports images   from my_imdir
...
# -det my_det -- imports my_det.cat & .fits instead of detectionImage
# -ext opt -- imports detection_opt.cat & .fits and clus_filt_opt.cat
# -opt -- optical
# -nir -- NIR
...
indet = params.get('det', 'detectionImage')
inext  = '_sci.cat'
#inextsex  = '_sci.inpar'

if 'opt' in params.keys():
    indet = 'detection_opt'
    inext = '_opt.cat'
    outrootdir = join(outrootdir, 'opt')
elif 'nir' in params.keys():
    indet = 'detection_nir'
    inext = '_nir.cat'
    outrootdir = join(outrootdir, 'nir')
elif 'ext' in params.keys():
    ext = params['ext']
    indet = 'detection_' + ext
    inext = '_%s.cat' % ext
    outrootdir = join(outrootdir, ext)
...
detcat = loadsexcat0(indet+'.cat', dir=indir)
...
if not eBV:
    ...
    detectionImage = join(imdir, indet+'.fits')
    ...
    if not exists(detectionImage):
        ...
        detectionImage = join(imdir, indet+'.fits.gz')
