...
def cipphot(files, filts, lamdict, nexposures, outdir, sexdir, detdir, cipdir, field, datestr, pixelscale=0.065, forcesec=True, forcetot=False, doapcor=True, eBV=None, apertype='iso', outname='', pruneCRs=True):
...
""" % (fieldv, datestr2, apertext, eBV)
    ...
    if not pruneCRs:
        header[2] = '## Not pruned: a few < 5-sigma detections may remain (flagsig > 0)\n'
