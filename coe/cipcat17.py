...
def findimages(searchstr, indir, silent=1, keeptotal=0, keepcln=0):
    if not silent:
...
def patchimages(over=0):
    ...
    for whtfile in whtfiles:  # includes "total" images
        ...
        weight_block = scipy.signal.convolve(weight,        block, mode, old_behavior=False)
        dataconv     = scipy.signal.convolve(weight * data,   ker, mode, old_behavior=False) / weight_block
        weightconv   = scipy.signal.convolve(weight * weight, ker, mode, old_behavior=False) / weight_block
...
def sexsetup():
    ...
    if '-IR' in sys.argv:
        detimage    = detfiles['IR']
        detrmsimage = detrmsfiles['IR']
    elif '-ACS' in sys.argv:
        detimage    = detfiles['ACS']
        detrmsimage = detrmsfiles['ACS']
    else:  # ACS+IR default
        detimage    = detfiles['ACSIR']
        detrmsimage = detrmsfiles['ACSIR']
...
if __name__ == '__main__':
    ...
    outfile = cipphot.cipphot(drzfiles, filts, filter_centers, nexposures, catdir, sexdir, detdir, cipdir, field, datestr, pixelscale=0.065, forcesec=True, forcetot=False, doapcor=doapcor, eBV=None, apertype=apertype, pruneCRs=pruneCRs)
