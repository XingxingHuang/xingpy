...
# id: integer
def cipweb(field, id, IRdet=False, justlinks=False, zspec=None, redo=False):
...
def cipwebs(infields, ids, IRdet, outpage, pagetitle=None, justlinks=False, zspecs=None, redo=False):
    if outpage:
    ...
    for i in range(len(ids)):
        ...
        if zspecs == None:
            ...
            zspec = zspecs[i]
        print 'cipweb', field, id, IRdet, type(field), zspec
        outpage1, pagetitle1 = cipweb(field, id, IRdet, justlinks=justlinks, zspec=zspec, redo=redo)
        if outpage:
...
if '.' in field:  # filename!
    ...
    cipwebs(infields, ids, IRdet, outpage, pagetitle, justlinks, zspecs, redo=redo)
else:
    args = sys.argv[2:]
    ...
    if '.' in id:
        ...
        cipwebs(infields, ids, IRdet, outpage, pagetitle, justlinks, redo=redo)
    elif ',' in id:
        ids = stringsplitatoi(id, ',')
        infields = [field] * len(ids)
        cipwebs(infields, ids, IRdet, outpage, pagetitle, justlinks, redo=redo)
    else:
        zspec = params_cl().get('ZSPEC', None)
        id = strendnum(id)
        id = int(id)
        outpage1, pagetitle1 = cipweb(field, id, IRdet, redo=redo, zspec=zspec)
