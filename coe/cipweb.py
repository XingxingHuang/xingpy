...
def cipweb(field, id, IRdet=False, justlinks=False, zspec=None, redo=False, altid=None, crosshair=None):
    ...
    if not justlinks:
        ...
        if crosshair <> None:
            cmd += ' -CROSSHAIR %d,%d' % tuple(crosshair)
...
def cipwebs(infields, ids, IRdet, outpage, pagetitle=None, justlinks=False, zspecs=None, redo=False, altids=None, crosshair=None):
    if outpage:
        ...
        fout.close()
    if crosshair:
        xcross, ycross = loaddata(crosshair).T
    for i in range(len(ids)):
        ...
        if altids == None:
            ...
            altid = altids[i]
        if crosshair:
            crosshair1 = xcross[i], ycross[i]
        else:
            crosshair1 = None
        print 'cipweb', field, id, IRdet, type(field), zspec, altid, crosshair1
        outpage1, pagetitle1 = cipweb(field, id, IRdet, justlinks=justlinks, zspec=zspec, redo=redo, altid=altid, crosshair=crosshair1)
        if outpage:
...
if '.' in field:  # filename!
    ...
    if '.' in id:
        ...
        id = strendnum(id)
        id = int(id)
        outpage1, pagetitle1 = cipweb(field, id, IRdet, redo=redo, zspec=zspec, altid=altid, crosshair=crosshair)
