...
class meanstd_robust:
    ...
    def __init__(self,x,n_sigma=3,n=5,sortedalready=False,verbose=False):
        self.x=x
    ...
    def run(self):
        ...
        for i in range(self.n):
            ...
            if self.verbose:
                ...
                #print imed
                #print ilo, ihi,
                print 'previous length', nx
                print 'current  length', nnx
                print 'median', aver
                print 'std', std1
                print 'n_sigma', self.n_sigma
                print 'range', lo, hi
                print 'indices', ilo, ihi
                print self.xsort[ilo]
                print self.xsort[ihi]
                print
        ...
        self.remaining = xrem = self.xsort[ilo:ihi]
        self.mean = mean(xrem)
        self.std  = rms(xrem - self.mean)
        self.noutliers = len(self.xsort) - len(xrem)
...
class madstd_robust:
    ...
    def run(self):
        ...
        #self.remaining = xrem = xs[ilo:ihi]
        self.remaining = xrem = self.xsort[ilo:ihi]
        self.median = median(xrem)
        self.mean = mean(xrem)
        #self.std  = rms(xrem - self.mean)
        self.std  = std_mad(xrem)
        self.noutliers = len(self.xsort) - len(xrem)

#s = meanstd_robust(dz)
#s.n = 0  # no sigma clipping
#s.run()
#m = s.mean
#r = s.std
...
# http://stackoverflow.com/questions/2777907/python-numpy-roll-with-padding
...
def roll_zeropad(a, shift, axis=None):
    ...
    if abs(shift) > n:
        ...
        shift += n
        zeros = zeros_like(a.take(arange(n-shift), axis))
        res = concatenate((a.take(arange(n-shift,n), axis), zeros), axis)
    else:
        zeros = zeros_like(a.take(arange(n-shift,n), axis))
        res = concatenate((zeros, a.take(arange(n-shift), axis)), axis)
    if reshape:
...
def datashift(data, dx=0, dy=0):
    #return data.roll(dy, axis=0).roll(dx, axis=1)
    data = roll_zeropad(data, dy, axis=0)
    data = roll_zeropad(data, dx, axis=1)
    return data
...
def datacrop(obj, border=0, outrange=True):
    """Crops data to region with non-zero data"""
    sumy = obj.sum(axis=1)
    ny = len(sumy)
    iy = arange(ny)
    iy = compress(sumy, iy)
    ylo, yhi = yr = minmax(iy)
    ylo = ylo - border
    yhi = yhi + border
    ...
    ix = arange(nx)
    ix = compress(sumx, ix)
    xlo, xhi = xr = minmax(ix)
    xlo = xlo - border
    xhi = xhi + border
    
    objcrop = obj[ylo:yhi+1,xlo:xhi+1]
    if outrange:
        return objcrop, ylo, yhi, xlo, xhi
    else:
        return objcrop
...
def multipleslogd(lo, hi, d=arange(9)+1):
    """multipleslogd(0.1, 10, (1,3)) = 0.1, 0.3, 1, 3, 10"""
    xx = []
    loglo = floorint(log10(lo))
    loghi = ceilint( log10(hi))
    for l in multiples(loglo, loghi):
        for n in d:
            x = n * 10**l
            if lo <= x <= hi:
                xx.append(x)
...
def multipleslogb(lo, hi, base=10):
    """multipleslog(2, 300) = 10, 100"""
    return base ** multiples(logb(lo, base), logb(hi, base)).astype(float)
    #return 10 ** multiples(log10(lo), log10(hi)).astype(float)
...
def rescale1(x, lo, hi):
    return rescale((x,), (lo, hi))[0]

def rescale2(x, lo, hi):
    return rescale(x, (lo, hi))
...
def xsig(x, P, nsigma=1, sided=1, peak=0):
    """given probability distribution P(x), find nsigma bounds on x"""
    ilr = Psig(P, nsigma=nsigma)
    n = len(x)
    ii = arange(n)
    xlr = interp(ilr, ii, x)
...
# P = erf(nsigma / sqrt(2))
# erfinv(P) = nsigma / sqrt(2)
# nsigma = erfinv(P) * sqrt(2)

def gausssig(P):
    """P bounded by +/-nsigma in Gaussian
    gausssig(0.682689) = 1"""
    return erfinv(P) * sqrt(2)
...
def gaussbtw(nsig1, nsig2=None):
    """FRACTION BETWEEN nsig1, nsig2"""
    if nsig2 == None:  # or could have just used sigma = gaussin
        nsig2 = -nsig1
...
def gauss2d(nsigma=1):
    """FRACTION WITHIN nsigma in 2-D Gaussian
    gaussin(1) = 0.682689"""
    return 1 - exp(-nsigma**2 / 2.)

def gauss2dsig(P):
    """P bounded by +/-nsigma in 2-D Gaussian
    gausssig(0.682689) = 1"""
    dchisq = -2 * log(1 - P)
    return sqrt(dchisq)

def percentile(p, x, s=1):
...
def sigrange1(x, nsigma=1):
    y = percentile(gaussp(nsigma), x)
    return y

def sigranges(x, nsigma=(-3,-2,-1,0,1,2,3), sorted=False):
    if not sorted:
        x = sort(x)
    nsigma = array(nsigma)
    p = gaussp(nsigma)
    n = len(x)
    ii = p * (n - 1.)
    v = interp(ii, arange(n), x)
    return v


def colorsigs(f1, df1, f2, df2, nsigma=(-3,-1,0,1,3), n=1e5, fratclip=-10):
    """Color (magnitude difference) of two objects at various sigma (probability)
    assuming Gaussian errors in flux.
    May need higher n=1e7 for more robust results"""
    ff1 = df1 * normal(size=n) + f1
    ff2 = df2 * normal(size=n) + f2
    cc = -2.5 * log10clip(ff1 / ff2, fratclip)  # -10 fratclip -> 25 max color
    ccs = sigranges(cc, nsigma)
    return ccs
...
def within(A, xc, yc, ro, yesorno=0, silent=0):  # --DC
...
def magnify2(a, n, k=3):
    """Magnifies (or demagnifies) an array [a] by a factor [n]
    using a spline interpolant of order [k].
    Output size is not simply original size times n.
    As in IRAF, the pixel spacing is multiplied by n.
    If you multiply by 2, you'll add a pixel between each of the existing ones,
    but the end points will stay the same.
    So 2 x 3 = 5.
    Similarly, 5 x 0.5 = 3."""
    ny, nx = a.shape
    x = arange(nx)
    ...
    XX, YY = meshgrid(X, Y)
    Z = a
    outgrid = RectBivariateSpline(y,x,a,kx=k,ky=k)
    AA = outgrid.ev(YY.flat, XX.flat)
    AA.shape = YY.shape
    return AA
...
def interp01(x, ydata, silent=0, extrap=0):
    return interp(x, (0,1), ydata)

def interp01b(x, ylo, yhi, silent=0, extrap=0):
    return interp(x, (ylo,yhi), (0,1))

def interplog1(x, ydata, silent=0, extrap=0):
    return 10 ** interp(x, (0,1), log10(ydata))

def interplog(x, xdata, ydata, silent=0, extrap=0):
    #return 10 ** interp(log10(x), log10(xdata), log10(ydata), silent=silent, extrap=extrap)
    return 10 ** interp(log10(x), log10(xdata), log10(ydata))
...
def weightedmeanunc(x, dx):
    """measurements x +/- uncertainties dx
    return weighted mean and its uncertainty"""
    x  = array(x).astype(float)
    dx = array(dx).astype(float)
    w  = 1 / dx**2
    x0 = sum(x * w) / sum(w)
    d = w / sum(w) * dx
    dx0 = hypotn(d)
...
meanunc = wmeanunc = weightedmeanunc
...
def cummean(m, axis=0):
    a = cumsum(m, axis=axis)
    n = arange(len(m)) + 1.
    return a / n
