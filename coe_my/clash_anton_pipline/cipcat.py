...
if __name__ == '__main__':
    ...
    tempdrzfiles = findimages(searchstr, indir, None, detdate=1)  # get datestr
    if datestr == 'datestr':
        datestr = imdatestr
    ...
    if catuptodate and not redocat:
    ...
    #print 'Sending catalog output to', catsubdir
    ...
    #if not exists(photfile) or ('-redocat' in sys.argv) or ('-rerun' in sys.argv):
    if not exists(photfile) or redocat or ('-rerun' in sys.argv):
        ...
        outfile = cipphot.cipphot(drzfiles, filts, filter_centers, nexposures, catdir, sexdir, detdir, cipdir, field, datestr, pixelscale=0.065, forcesec=True, forcetot=False, doapcor=doapcor, eBV=None, apertype=apertype, pruneCRs=pruneCRs, detcam=detcam)   # by xingxing  do not use extinction correction eBV = None
