...
try:
    ...
    pywcsloaded = False
    #pass # print "pywcs not installed, so not importing it"
...
#################################
# http://stsdas.stsci.edu/astrolib/pywcs/examples.html
# RA, Dec <--> x, y

def sky2xy(RA, Dec, infits, RAunits='deg'):
    header = pyfits.open(infits, memmap=1)[0].header
    wcs = pywcs.WCS(header)
    if RAunits == 'h':
        RA = 15 * array(RA)  # hours -> deg
    sky = array([RA, Dec])
    x, y = wcs.wcs_sky2pix(sky.T, 1).T  # with (1,1) at the origin
    return x, y

def xy2sky(x, y, infits, RAunits='deg'):
    header = pyfits.open(infits, memmap=1)[0].header
    wcs = pywcs.WCS(header)
    xy = array([x, y])
    RA, Dec = wcs.wcs_pix2sky(xy.T, 1).T  # with (1,1) at the origin
    if RAunits == 'h':
        RA = RA / 15  # deg -> hours
    return RA, Dec
...
def mkdir(newpath, mode=None):  # 0775
    """Make a directory path and set permissions along the way"""
    path = ''
    for dir in splitdirs(newpath):
        path = os.path.join(path, dir)
...
def xy2sky(x, y, infits, options='', silent=0, cleanup=1):
    data = array([x, y])
    tempxy = 'temp.xy'
    tempwcs = 'temp.wcs'
    savedata(data, tempxy+'+')
    cmd = 'xy2sky %s %s @%s > %s' % (options, infits, tempxy, tempwcs)
    if not silent:
        print cmd
    
    os.system(cmd)
    RA, Dec, J2000, x, y = loadcols(tempwcs)  # RA Dec J2000 x y
    if cleanup:
        os.remove(tempxy)
        os.remove(tempwcs)
...
if pyfitsloaded:
    ...
    if not pyfitsusesnumpy:
        ...
        #import numarray
        from numpy import numarray # by xingxing
...
def savedata(data, filename, dir="", header="", separator="  ", format='', labels='', descriptions='', units='', notes=[], pf=0, maxy=300, machine=0, silent=0):
    ...
    if dow and os.path.exists(outfile):
        ...
        if format:
            ...
            colstr = [0] * nx  # LENGTH OF STRING

            # Go through data (up to maxy rows) and determine type and length of each column
            if machine:
                maxy = 0
            ...
            for iy in yyy:
                for ix in range(nx):
                    datum = data[iy,ix]
                    if type(datum) in (type('s'), type(sarr[0])):
                        ns = len(datum)
                        colstr[ix] = max([ ns, colstr[ix] ])
                    else:
                        if isNaN(datum):
                            ni, nd = 1, 1
                        else:
            ...
            for ix in range(nx):
                if colexp[ix]:
                    ...
                    collen = colstr[ix] + colint[ix] + coldec[ix] + (coldec[ix] > 0) + (colneg[ix] > 0)  # EXTRA ONES FOR DECIMAL POINT / - SIGN
                if labels and not machine:
                    collen = max((collen, len(labels[ix])))  # MAKE COLUMN BIG ENOUGH TO ACCOMODATE LABEL
                collens.append(collen)

            format = '  '  # Add extra space comensurate with ##
            for ix in range(nx):
                collen = collens[ix]
                format += '%'
                ...
                if colstr[ix]:
                    format += '%ds' % colstr[ix]
                elif colexp[ix]:  # REALLY BIG (EXP FORMAT)
                    format += '.3e'
                else:
                    if coldec[ix]:  # FLOAT
        ...
        if machine:  # machine readable
            ...
            for ix in range(nx):
                ...
                cf = string.strip(cf)
                format += cf
                # convert format character to machine readable counterpart
                mformat += {'d':'I', 'f':'F', 'e':'E', 's':'A'}[cf[-1]]
                mformat += cf[:-1]
                if ix < nx - 1:
                    format += separator
                    mformat += separator
                ...
                if string.find(cf, 's') > -1:
                    cf = string.split(cf, 's')[0]  # STRING: Number before 's'
                    collen = string.atoi(cf)
                elif string.find(cf, 'e') > -1:
                    collen = 9 + colneg[ix]
                else:
...
##                 descriptions = labels
            if labels:
                ...
                if not header:
                    ...
                    if header[-1] == '-':  # SPECIAL CODE TO KEEP ONLY BEGINNING ## LINES OF HEADER
                        #header = header[:-1]
        ...
        if machine:  # Left justify strings
            for ix in range(nx):
                if colstr[ix]:
                    for iy in range(ny):
                        #print ix, iy, colstr[ix], data[iy,ix]
                        data[iy,ix] = data[iy,ix].ljust(collens[ix])
...
class VarsClass:
    ...
    def assigndata(self):
        ...
        for iii in range(len(self.labels)):
            if iii > n-1:
                print 'COLUMN %d GREATER THAN NUMBER OF COLUMNS %d' % (iii+1, n)
                xxx[9] = 3
            label = self.labels[iii]
            try:
                cmd = 'self.%s = self.data[iii]' % label
                #print iii, self.data.shape
                #print cmd
                exec(cmd)
            except:
...
##             print "VarsClass: SUBSET CANNOT BE CREATED: good LENGTH = %d, data LENGTH = %d" % (len(self.id), len(good))
    ...
    def equal(self, labels, val, **other):
        return self.closeto(labels, val, **other)
    ...
    def get(self, label, orelse=None):
        if label in self.labels:
            ...
            out = orelse
        return out
    def set(self, label, data, format=None, description=None):
	if singlevalue(data):
    ...
    def add(self, label, data, format=None, description=None):
        ...
        if format <> None:
            self.formats[label] = format
    def assign(self, label, data, order=None, format=None, description=None):
        if label in self.labels:
            self.set(label, data, format=format, description=description)
        else:
            self.add(label, data, format=format, description=description)
        if order <> None:
            self.reorder(label, order)
...
##             pause()
    ...
    def reorder(self, label, order):
        if order < 0:
            order = len(self.labels) + order
        self.labels.remove(label)
        self.labels.insert(order, label)
...
def loadsexcat0(infile, dir='', silent=0):
    ...
    #print ncols
    #print sum(pcols)
    pcols = concatenate((pcols, [ncols-sum(pcols)]))

    allparams = []
    for i in range(len(cols)):
        #print pcols[i], params[i]
        if pcols[i] == 1:
            allparams.append(params[i])
        else:
            for j in range(pcols[i]):
                param = params[i] + '_%d' % j
                allparams.append(param)
    
    cat = loadcat(infile, labels=allparams)
    return cat
    ...
    for ii in range(len(bigparamnames)): #paramcol.keys():
        ...
        if pp in ['flags', 'fwhm', 'rf', 'mag']:
            ...
            if len(col) == 1:
                ...
                #print data.shape
                #print col-1
                #xxx = data.take(col-1, axis=0)
                #xxx = take(data, col-1)
                #print shape(xxx)
                #quit()
                #outdata.append(ravel(take(data, col-1)))
                outdata.append(take(data, col-1, axis=0))
...
def saveregions(x, y, filename, labels=[], x2=None, y2=None, precision=1, coords='image', color="green", symbol="circle", size=0, width=0, fontsize=12, bold=1):
