...
def loadfitsfiles(instrs='',drzstr = '_sci', whtstr = '_sci_weight'):  # by xingxing
    ...
    for drzfile in drzfiles:
        ...
        if countspersec ==1:
             ...
             datasum = where(rms, datasum + data / rms**2,datasum)   
        else:
              print 'countspersec ???'
              pause() 
        weightsum = weightsum + weight
        rms2sum = where(rms, rms2sum +1./rms**2, rms2sum)
        totexptime += exptime
...
def sexsetup():
    ...
    pardict['PHOT_APERTURES'] = '6'  # add by xingxing.  So you can use the detectionImage_APER.fits now
    if useweight != 0:
    ...
    for filtimage in drzfiles:
        ...
        for key in sort(pardict.keys()):
            ...
            if key == 'CHECKIMAGE_NAME':
                ...
                if useweight != 0:
                    val  = '%s,%s' % (detwhtimage, whtimage)
            elif key == 'WEIGHT_TYPE':
                val = 'MAP_RMS,MAP_RMS'
                if useweight != 0:
                    val  = 'MAP_WEIGHT,MAP_WEIGHT'            
...
if __name__ == '__main__':
    ...
    cipoutdir = join(datadir, 'clash_cat','cipphot')
    ...
    #################################
    # OUTPUT DIRECTORIES
    rootdir = join(os.path.dirname(datadir), 'clash_cat' ,field)   
    outrootdir = rootdir
    ...
    if exists(catdir):
        ...
        datedirs = glob.glob(os.path.join(catdir,'1?????'))
        dd = []
        for d in datedirs:
            try:
                tmp = d.split('/')[-1]
        ...
        if not redocat:
            catfiles = glob.glob(os.path.join(catdir,catdatestr,'*.cat*'))
    ...
    rmsfiles = [redir(wfile.replace(rmsstr,'_rms'),rmsdir )for wfile in whtfiles]

    #################################
    # RMS FILES
    useweight = params.get('useweight', 0)
    global useweight     # by xingxing
    ...
    imroot  = field   # prefix of the name
    imext = '.fits'     # prefix of the name
    detfiles = {}
    detrmsfiles = {}
    detwhtfiles = {}
    ...
    for detfile in allfiles:
     ...
     # by xingxing
    drzstr = '_drz' # do not change
    whtstr = '_wht'  # do not change 
    drzstr = params.get('drzstr',drzstr)
    if  detcam=='ACSIR':
        print 'Creating ACS+IR detection image'
        outdrz = imroot + '_acs_wfc3ir_total_drz' + imext
        detfiles['ACSIR'] = join(imcopydir,outdrz)
        rmsfile = outdrz.replace('drz', 'rms') # by xingxing
        rmsfile = redir(rmsfile, rmsdir)
        detrmsfiles['ACSIR'] = join(rmsdir,rmsfile)
        outwht = outdrz.replace('_drz', '_wht')
        outdir = imcopydir
        outdrz = join(outdir,outdrz)
        outwht = join(outdir,outwht)
        detwhtfiles['ACSIR'] = outwht
        makedetectionimage(['acs', 'wfc3'], outdrz, outwht,drzstr = drzstr, whtstr = whtstr)
    ...
    if 0:
        ...
        makedetectionimage('acs', 'ACS_detectionImage.fits')
        makedetectionimage(['acs', 'wfc3ir'], 'ACSIR_detectionImage.fits')
