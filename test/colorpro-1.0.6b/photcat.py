...
#  PRIOR TO FIX, isocr & ext WERE ONLY APPLIED TO blurry AND FIRST NON-blurry FILTER
#   (NOTE: I verified that the UDF was analyzed correctly 
#    with cat3.py & cor7.py, predecessors to sexsegcat.py/photcat.py)
# --Thanks to Elinor Medezinski for noticing the strange behavior!
#
# ColorPro 1.0.4: sexsegcat.py NAME CHANGED TO photcat.py
#
...
def photcat(filts, detfilt, mapped2Afilts, blurryfilts, zpdict, extdict, outfile):
    # -> FILE NAMES
...
##         detfilt2 = detfilt
    ...
    for filt in filts:
        ...
        if inAframe:
            ...
            filt2 = filt + '2A'
        #print 'filtb, 2:', filtb, filt2
        
        # LOAD SExSeg CATALOG: FLUX -> MAG
        cat = loadcat(filt2+'.cat')
        ...
        if not blurry:
            ...
            dtocat = loadcat(detfilt2+'to'+filt2+'.cat')
            if detcat.len() <> dtocat.len():
                print 'CATALOGS HAVE DIFFERENT LENGTHS IN photcat.py'
                print detcat.len(), detfilt+'.cat'
                print dtocat.len(), detfilt2+'to'+filt2+'.cat'
                sys.exit()
            detmag, detmagerr = sex2bpzmags(dtocat.fluxiso, dtocat.fluxerriso, zpdict[detfilt])

        if detauto.shape <> detmag.shape:
            print 'detauto & detmag DIFFERENT LENGTHS IN photcat.py'
        ...
        if 1:  # (v1.0.5 BUG FIX)
            ...
            elif 0: # filt == detfilt:  # (v1.0.5)
                magerr = where(maggood, deterrauto, magerr)
                # COLOR UNCERTAINTIES COME FROM ISO UNCERTAINTIES
                # detauto ALWAYS SUBTRACTS OUT
    ...
    # SAVE
    outcat.save('phot.cat', format=format)  # sexseg.cat
