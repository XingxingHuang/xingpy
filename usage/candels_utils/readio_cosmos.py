...
# COSMOS/UltraVISTA
CU_cat  = {\
      'cosmos': 'COSMOS-catalog/UVISTA_rselected_v4.5.cat'\
        }      
...
#________________________________________________________________________________________________  candels
def readGSD(test=0):
    '''
...
def get_key_band2(keys,bands,special='f_'):
    ...
    for band in bands:
        key = special+band
        if string.lower(key) in keys or string.upper(key) in keys:
...
#________________________________________________________________________________________________    COSMOS_UltraVISTA
def cat2fitsCU(field):
    '''
     .cat > .fits
    '''
    import  asciidata 
    # catfile = 'UVISTA_rselected_v4.5.cat'
    catfile = join(rootdir,CU_cat[field])
    fitsfile = catfile.replace('.cat','.fits')
    data = asciidata.open(catfile)
    data.writetofits(fitsfile)
    print 'File saved: ',fitsfile
    
    #from pyfits import Column
    f = pyfits.open(fitsfile)
    data = f[1].data
    names = 'id ra dec xpix ypix rp_tot erp_tot rp erp Ks eKs H eH J eJ Y eY ch4 ech4 ch3 ech3 ch2 ech2 ch1 ech1 zp ezp ip eip V eV gp egp B eB u eu IA484 eIA484 IA527 eIA527 IA624 eIA624 IA679 eIA679 IA738 eIA738 IA767 eIA767 IB427 eIB427 IB464 eIB464 IB505 eIB505 IB574 eIB574 IB709 eIB709 IB827 eIB827 fuv efuv nuv enuv mips24 emips24 K_flag K_star K_Kron apcor z_spec z_spec_cc z_spec_id star contamination nan_contam flux_rad orig_cat_id orig_cat_field USE'.split()
    f[1].data.names=names
    for i in range(len(names)):
        txt = 'TTYPE%i' %(i+1)
        f[1].header[txt] = names[i]
    f.writeto('tmp.fits')
    f.close()    
...
def readCU(field):
   ...
   catfitsfile = catfile.replace('.cat','.fits')
   print 'Loading ',catfitsfile    
   data = pyfits.getdata(catfitsfile,1)
   return(data)
