...
rootdir_xing = '/Users/xing/data/clash_cat_anton'
...
cat_image = join(infodir,'image.cat')
cat_model = join(infodir,'model.cat')
cat_catlog = join(infodir,'catalog.cat')
cat_catlog_xing = join(infodir,'catalog_xing.cat')
cat_cluster = join(infodir,'cluster.cat')
...
def cluster_dir(cluster):
    # return image dir
    info  = loaddict(cat_image)[cluster]
    imgdir = join(rootdir,info[0])
    return(imgdir)    

#def getcat(cluster,special = 'IR'):
#        infos = loaddict(cat_cluster)  
#    info = infos[cluster]
def getcat_xing(cluster,special=None):
    '''
    return the path and catalog
    '''
    infos = loaddict(cat_catlog_xing)  
    info = infos[cluster]
    cat_name = join(rootdir_xing,info[0],info[1])
    if special!=None:
...
def getimage(cluster,special=None,imagedir = '',wht=0,rms=0,arrayonly=0):
    '''
    return the path to CLASH images for the cluster
    if imagedir is specified, the things in imagedir will be loaded.
    '''
    ...
    if imagedir =='':
       ...
       imgdict = loaddict(imagedir)
    info = imgdict[cluster]
    if wht!=0:
        image_name = join(rootdir,info[0],info[2])
    if rms!=0:
        image_name = join(rootdir,info[0],info[3])
    else:    
    ...
    # return array or dictionary
    if arrayonly !=0:
        out_array = []
        for key in image_files.keys():
            out_array.append(image_files[key])
        return(out_array)   
    return(image_files) 
