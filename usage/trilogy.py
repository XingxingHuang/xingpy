...
# modified for non-interactive mode WZ Jun 2011
##############################################################
...
class Trilogy:
    def __init__(self, infile=None, images=None , output=None, imagesorder='BGR', **inparams):
        ...
        self.outdir=output  # add by xingxing
        if self.outdir:
           self.inkeys.append(self.outdir)  # add by xingxing
    ...
    def determinescalings(self):
        ...
        while redo:  # Until user is happy with test image of core
            ...
            #unsatpercent = 1 - 0.01 *array([0.0001,0.0001,0.001] ) # deal with each channel separately by xingxing 
            for ichannel, channel in enumerate(self.mode):
                self.levdict[channel] = determinescaling(stampRGB[ichannel], unsatpercent)  
                #self.levdict[channel] = determinescaling(stampRGB[ichannel], unsatpercent[ichannel])
            ...
            if self.testfirst:
                ...
                print 'Otherwise, enter new values:  self.noiselum self.satpercent self.colorsatfac   redo'
    ...
    def makecolorimage(self):
        ...
        if self.show:
            try:
                os.system('open ' + outfile)   #only runs in MAC, so I skip this!  by xingxing 
...
if __name__ == '__main__':
    ...
    if not os.path.isfile(infile):
        txt = 'Try this command to create the default parameter file:\n'
        txt += '       cp  /Users/xing/programs/xing/coe_my/trilogy/trilogy.in  ./'
        sys.exit(txt)
