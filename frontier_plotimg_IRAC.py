...
# --------------------------------------------------------------------- 
# catalog
catfile 	= './obj.cat'			# the catalog where ra dec can be found
...
indir 	 	= '/Users/xing/data/irac_img/*/Spitzer/mosaics'				# the directory for images
imageprefix	= ['*_mosaic.fits']				# The program will grab the images that match the name here. PLEASE also make sure the 'bands' defined bellow also included in the names of images.
...
# tmp_0_0.fits
outdir		= './img/'
bands   	= ['ch1','ch2']		# the bands of output images. PLEASE make sure these bands are included in the bands_all(check the default setting).
boxsize 	= 15					# the size of output images. the final size will be 2*boxsize+1
...
class ImageForCatalog:
    ...
    def plot_sep_img(self):
        ...
        for index in range(len(self.obj)):
            #  add in Mar 11 for this purpose
            self.otherprefix = self.obj[index]
            self.imglist = self.get_imglist()
            ##
            obj = self.obj[index]
            ra  = self.ra[index]
            ...
            for bi in range(len(self.bands)):
                iraf.stsdas.toolbox.imgtools.rd2xy(self.imglist[bi],ra,dec,hour='no')
                ...
                print 'imcopy(%s, %s)'  %(infile, outfile)
                print
    ...
    def plot_all_img(self):
        ...
        for index in range(len(self.obj)):
            ...
            max_iter = 50
            min_val = 0.0
            non_linear_fact = 30.0
            sky, num_iter = img_scale.sky_median_sig_clip(data, sig_fract, per_fract, max_iter)
            img_data_b = data - sky
            b = img_scale.asinh(img_data_b, scale_min = min_val, non_linear=non_linear_fact)
            im = imshow(b, vmin=0, vmax=1,origin='lower')
            pdb.set_trace()
            plt.colorbar()
            '''
            # scale method  for ch1 ch2 image only
            sig_fract = 3.0
            per_fract = 5.0-4
            max_iter = 50
            min_val = 0.0
            non_linear_fact = 30.0
            data1 = data[0:box*2+1,0:box*2+1]
            data2 = data[0:box*2+1, (box*2+10):(box*4+10+1)]
            # data1
            tmp = data1
            sky, num_iter = img_scale.sky_median_sig_clip(tmp, sig_fract, per_fract, max_iter)
            img_data_b = tmp - sky
            b1 = img_scale.asinh(img_data_b, scale_min = min_val, non_linear=non_linear_fact)
            # data2
            tmp = data2
            sky, num_iter = img_scale.sky_median_sig_clip(tmp, sig_fract, per_fract, max_iter)
            img_data_b = tmp - sky
            b2 = img_scale.asinh(img_data_b, scale_min = min_val, non_linear=non_linear_fact)
            # add
            #pdb.set_trace()
            b = data*0.
            #pdb.set_trace()
            b[0:box*2+1, 0:box*2+1] = b1
            b[0:box*2+1, (box*2+10):(box*4+10+1)]= b2           
            im = imshow(b, vmin=0, vmax=0.1,origin='lower')
            plt.colorbar()
            ...
            for index in range(len(self.bands)):
                #lx = 12+(box*2+10)*(j%8)
                #ly = (box*2+10)*(j//8)
                lx = (box*2+10)*index+box+1
                ly = box
                ax.text(lx,0,string.upper(self.bands[index]),color='red',ha='center') #,fontweight='bold',ha='center')    
                ax.plot(lx,ly,'o',color='None', markeredgecolor='fuchsia', markersize=50, alpha=0.6,markeredgewidth=2)     
            # save
    ...
    def plot_web_img(self):
        ...
        for index in range(len(self.obj)):
            ...
            if not os.path.isfile(os.path.join(outdir,outspec)):
                 print 'WARNING: spectral figures not found!! ', outspec    
                 nospec =1
            else:
                 nospec = 0    
