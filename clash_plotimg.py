...
# --------------------------------------------------------------------- 
# catalog
catfile 	= 'a2744_zheng.cat'			# the catalog where ra dec can be found
NAMEnum		= 1					# the column of source names. This will be used for the output file names
RAnum 		= 3					# the column of ra dec
DECnum 		= 4

# redshift of nearby objects 
include_bpz 	= 1					# include the bpz of nearby objects or not
...
indir 	 	= './a2744_new'				# the directory for images
imageprefix	= '*_sci.fits*'				# The program will grab the images that match the name here. PLEASE also make sure the 'bands' defined bellow also included in the names of images.
...
# tmp_0_0.fits
outdir		= './tmp'
bands   	= ['f435w','f606w','f814w','f105w','f125w','f140w','f160w']			# the bands of output images. PLEASE make sure these bands are included in the bands_all(check the default setting).
boxsize 	= 50					# the size of output images. the final size will be 2*boxsize+1
namepre		= 'a2744'				# the prefix of output images
...
class ImageForCatalog:
    ...
    def get_imglist(self):
        ...
        '''
        # glob image list
        imgfile = glob.glob(os.path.join(self.indir,self.imageprefix))
        # otherprefix
        ...
        for band in self.bands:
            ...
            if band_find == 0 :
                sys.exit('ERROR: '+band+' image is not found in the '+indir)
            if band_find >1 :
    ...
    def plot_all_img(self):
        ...
        for index in range(len(self.obj)):
            ...
            if self.includeindex ==1 :
                ...
                outname3 = '%s_%s_%s_z.%s' %(namepre,count,obj, 'png')
            else:
            ...
            if os.path.isfile(outfile):
                os.rename(outfile,outfile+'_bck')
            iraf.imcombine(infile,outfile,offsets="grid "+str(gridnum)+" "+str(box*2+10)+" 2 "+str(box*2+10))  
            ...
            for index in range(len(self.bands)):
                #lx = 12+(box*2+10)*(j%8)
                #ly = (box*2+10)*(j//8)
                lx = (box*2+10+1)*index+box
                ly = box
                ax.text(lx,0,string.upper(self.bands[index]),color='red',ha='center') #,fontweight='bold',ha='center')    
                ax.plot(lx,ly,'o',color='None', markeredgecolor='fuchsia', markersize=9, alpha=0.6,markeredgewidth=2)     
            # save
